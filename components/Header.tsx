    ...
    <header className="bg-brand-navy text-white relative">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-center gap-4 h-14 overflow-visible relative">
        <Link href="/" className="flex items-center shrink-0 relative z-10">
          <span className="relative w-20 h-20 shrink-0 mt-4 ml-4">
        ...
        <form action="/#products" className="flex-1 max-w-2xl">
          <div className="relative">
            <input type="text" placeholder="What are you looking for?" className="w-full rounded-md pl-4 pr-10 py-2 text-brand-navyDark text-sm focus:outline-none" />
            <button type="submit" aria-label="Search" className="absolute right-0 top-0 h-full px-3 flex items-center justify-center text-brand-navyDark">
              <SearchIcon className="w-5 h-5" />
            </button>
          </div>
        </form>

        <a href={whatsappHref} target="_blank" rel="noopener noreferrer" aria-label="Buy via WhatsApp" className="hidden sm:flex items-center gap-2 shrink-0 bg-green-600 hover:bg-green-500 text-white text-sm font-semibold px-4 py-2 rounded-full transition whitespace-nowrap">
          <WhatsAppIcon className="w-5 h-5" />
          <span>Buy via WhatsApp</span>
        </a>
        <a href={whatsappHref} target="_blank" rel="noopener noreferrer" aria-label="Buy via WhatsApp" className="sm:hidden flex items-center justify-center shrink-0 bg-green-600 hover:bg-green-500 text-white p-2 rounded-full transition">
          <WhatsAppIcon className="w-5 h-5" />
        </a>

        <div className="relative shrink-0">
          <button onClick={() => (isOpen ? closeCart() : openCart())} aria-label="Open cart" className="hidden md:flex items-center gap-2 text-sm hover:text-brand-yellow transition">
            <CartIcon className="w-6 h-6" />
            <span>{totalItems} {totalItems === 1 ? "Item" : "Items"} · {formatPrice(totalCents)}</span>
          </button>
          <button onClick={() => (isOpen ? closeCart() : openCart())} aria-label="Open cart" className="md:hidden flex items-center gap-1">
            <CartIcon className="w-6 h-6" />
            {totalItems > 0 && <span className="bg-brand-orange text-white text-xs font-bold rounded-full px-1.5 py-0.5">{totalItems}</span>}
          </button>
        </div>
      </div>
      ...
      <div className="bg-brand-navy text-white text-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-center gap-10">
          ...
          <a href={`mailto:${EMAIL_ADDRESS}`} aria-label="Email Life Tools support" className="flex items-center gap-2 hover:text-brand-yellow transition"><EmailIcon className="w-5 h-5" /><span>Email</span></a>
        </div>
      </div>
    </header>
  );
