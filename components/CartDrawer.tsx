    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full sm:w-[420px] h-full bg-white shadow-xl flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="text-lg font-bold text-brand-navyDark">{t("yourCart")}</h2>
          <button
            onClick={closeCart}
            aria-label="Close cart"
            className="p-1 text-gray-500 hover:text-brand-navyDark"
          >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {items.length === 0 ? (
            <p className="text-sm text-gray-500 text-center mt-10">{t("cartEmpty")}</p>
          ) : (
        ...
        {items.length > 0 && (
          <div className="border-t px-5 py-4 space-y-3">
            ...
            <p className="text-xs text-gray-500 text-center">{t("checkoutNote")}</p>
          </div>
        )}
      </div>
    </div>
